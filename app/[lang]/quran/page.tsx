...
async function fetchSurahs(): Promise<Surah[]> {
  try {
    ...
    if (!json.data || !Array.isArray(json.data)) return FALLBACK_SURAHS;
...
export default async function QuranPage({ params }: { params: Promise<{ lang: string }> }) {
  const { lang: rawLang } = await params;
  const lang = toLang(rawLang);
  const isAr = lang === "ar";
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      ...
      <div className="text-center mb-10">
        ...
        >
          <BookOpen size={32} color="#1B5E20" />
        </div>
        <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-3">{t(lang, "quran.title")}</h1>
        <p className="text-gray-500 mb-4">{t(lang, "quran.subtitle")}</p>
        <p className="text-2xl font-quran text-gray-700 leading-loose">{t(lang, "quran.bismillah")}</p>
      </div>
